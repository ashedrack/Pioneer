...
from moto import mock_dynamodb, mock_ec2, mock_s3
...
class CloudPioneerE2ETest(unittest.IsolatedAsyncioTestCase):
    ...
    def setup_test_environment(self, aws_setup, tmp_path):
        ...
        # Create test config
        config = {
            "agent": {"collection_interval": 60, "shipping_interval": 10},
            "backends": {"aws": {"enabled": True, "region": "us-east-1"}},
            "collectors": {"system": {"enabled": True}},
        }
    ...
    async def setUp(self):
        ...
        self.s3 = boto3.client(
            "s3",  # Don't specify region for S3 client
            endpoint_url="http://localhost:4566",
            aws_access_key_id="testing",
            aws_secret_access_key="testing",
        )
    ...
    async def test_01_agent_installation(self):
        ...
        result = subprocess.run(
            ["./install.sh", "-k", self.api_key], capture_output=True, text=True
        )
    ...
    async def test_02_metric_collection(self):
        ...
        container = self.docker_client.containers.run(
            "nginx:latest", name="test-nginx", detach=True
        )
        ...
        self.assertTrue(any(m["name"] == "docker.container.cpu.usage" for m in metrics))
    ...
    async def test_03_ml_predictions(self):
        ...
        data = pd.DataFrame(
            {
                "value": [
                    50 + 30 * (i.hour / 24) + 10 * (i.dayofweek / 7) for i in dates
                ]
            },
            index=dates,
        )
        ...
        optimizer = ResourceOptimizer(
            {"utilization_threshold_low": 20, "utilization_threshold_high": 80}
        )
        ...
        task = scheduler.schedule_task(
            {
                ...
                "parameters": {"provider": "aws", "resource_type": "ec2"},
            }
        )
    ...
    async def test_10_api_integration_new(self):
        ...
        response = await self.client.get(
            "/api/v1/resources/recommendations/test-instance"
        )
        assert response.status_code == 200
        data = response.json()
        assert "resource_id" in data
...
class CloudPioneerE2ETest:
    ...
    def test_01_agent_installation(self, setup):
        ...
        with patch("src.agent.installer.AgentInstaller") as mock_installer:
            ...
            response = setup.post(
                "/api/v1/agent/install",
                json={"instance_id": "i-1234567890abcdef0", "region": "us-east-1"},
            )
    ...
    def test_06_agent_installation_new(self, setup):
        ...
        with patch("src.agent.installer.AgentInstaller") as mock_installer:
            ...
            response = setup.post(
                "/api/v1/agent/install",
                json={"instance_id": "i-0987654321fedcba0", "region": "us-west-2"},
            )
