    ...
    def get_optimization_metrics(self, resource_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'metrics': {
                    'utilization': {
                        'cpu': float(metrics['cpu_utilization'].mean()) if 'cpu_utilization' in metrics else None,
                        'memory': float(metrics['memory_utilization'].mean()) if 'memory_utilization' in metrics else None
                    },
                    'cost': {
                        'current': float(metrics['cost'].sum()) if 'cost' in metrics else None,
                        'projected': float(metrics['projected_cost'].sum()) if 'projected_cost' in metrics else None
                    }
                }
            }
        except Exception as e:
            self.logger.error(f"Failed to get optimization metrics for {resource_id}: {str(e)}")
            raise
