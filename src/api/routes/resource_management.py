...
from src.api.models import (MetricsRequest, MetricsResponse,
                            RecommendationsResponse, ScheduleRequest,
                            ScheduleResponse)
...
async def submit_metrics(
    request: MetricsRequest, predictor: MetricsPredictor = Depends(get_predictor)
) -> MetricsResponse:
    """Submit resource metrics"""
    try:
        metrics_data = request.model_dump()
        predictions = predictor.predict(metrics_data)
        return MetricsResponse(status="success", predictions=predictions)
...
async def get_metrics(
    resource_id: str, predictor: MetricsPredictor = Depends(get_predictor)
) -> Dict[str, Any]:
...
async def schedule_action(
    request: ScheduleRequest, scheduler: ResourceScheduler = Depends(get_scheduler)
) -> ScheduleResponse:
    """Schedule a resource action"""
    try:
        schedule_data = request.model_dump()
        scheduler.schedule_action(schedule_data)
        return ScheduleResponse(status="scheduled", action=schedule_data)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_schedule(
    ...
    try:
        ...
        return {"resource_id": resource_id, "scheduled_actions": actions}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_recommendations(
    ...
    try:
        ...
        return RecommendationsResponse(
            resource_id=resource_id, recommendations=recommendations
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
